...
if [ ! -d "$DATADIR/mysql" ]; then
	...
	# Start mysqld (mysql is the client). skip-networking disables TCP/IP.
	# Instead, mysql and mysqld communicate via /var/run/mysqld/mysqld.sock, which is a Unix domain socket
	...
	# Invokes the mysql command string above with this as stdin
	"${mysql[@]}" <<-EOSQL
